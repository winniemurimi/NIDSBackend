...
router.post('/predictKNN', upload.single('file'), async (req, res) => {
  const filePath = req.file.path;  // Path to the uploaded CSV file

  try {
      // Send the file to the FastAPI prediction endpoint
      const response = await axios.post('http://0.0.0.0:8000/predict/csv/', {
          headers: {
      ...
      // Send back the classification results
